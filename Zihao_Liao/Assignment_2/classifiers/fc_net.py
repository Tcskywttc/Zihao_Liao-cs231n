...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers + 1):
            Wi = f"W{i}"
            bi = f"b{i}"
            self.params[Wi] = weight_scale * np.random.randn(dims[i - 1], dims[i])
            self.params[bi] = np.zeros(dims[i])

        # gammas / betas for normalization (for layers 1..L-1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                gamma, beta = self.params[f"gamma{i}"], self.params[f"beta{i}"]
                out, norm_cache = layernorm_forward(out, gamma, beta, self.ln_params[i - 1])
        ...
        Wlast, blast = self.params[f"W{self.num_layers}"], self.params[f"b{self.num_layers}"]
        scores, final_cache = affine_forward(out, Wlast, blast)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
