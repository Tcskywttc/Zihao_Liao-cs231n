...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (1) features -> initial hidden state h0
        h0 = affine_forward(features, W_proj, b_proj)  # (N, H)

        # (2) word embeddings for captions_in
        x_embed = word_embedding_forward(captions_in, W_embed)  # (N, T-1, W)

        # (3) RNN over word embeddings
        h = rnn_forward(x_embed, h0, Wx, Wh, b)  # (N, T-1, H)

        # (4) temporal affine: hidden -> scores over vocab
        scores = temporal_affine_forward(h, W_vocab, b_vocab)  # (N, T-1, V)

        # (5) temporal softmax loss with mask
    ...
    def sample(self, features, max_length=30):
        ...
        # start token
        start_idx = self.word_to_idx.get('<START>')
        if start_idx is None:
            raise ValueError("word_to_idx must contain '<START>'")
        next_words = torch.full((N,), start_idx, device=device, dtype=torch.long)

        for t in range(max_length):
            # embed current words
            xt = W_embed[next_words]              # (N, W)
            # one RNN step
            h = torch.tanh(xt.matmul(Wx) + h.matmul(Wh) + b)  # (N, H)
            # vocab scores and pick next word (greedy)
            scores = h.matmul(W_vocab) + b_vocab              # (N, V)
