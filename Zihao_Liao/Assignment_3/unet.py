...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            # [ResnetBlock(dim_in->dim_in), ResnetBlock(dim_in->dim_in), Downsample(dim_in->dim_out)]
            ##################################################################
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ##################################################################
            # Account for skip connections: each ResnetBlock takes 2 * dim_out
            # channels as input (current + skip).
            ##################################################################
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        # eps_cfg = (scale + 1) * eps(x_t, cond) - scale * eps(x_t, empty)
        #
        # Call self.forward twice:
        #  - once with the given condition
        #  - once with text_emb = None for unconditional
        ##################################################################
        # Conditional pass
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        # UNet forward: Down -> Middle -> Up with skip connections
        ##################################################################
        skips = []

        # 1. Downsampling
        for block1, block2, down in self.downs:
            x = block1(x, context=context)
            skips.append(x)  # after first ResNet
            x = block2(x, context=context)
            skips.append(x)  # after second ResNet
            x = down(x)
        ...
        for up, block1, block2 in self.ups:
            ...
            # First ResNet in this up block
            skip = skips.pop()
            x = torch.cat([x, skip], dim=1)
            x = block1(x, context=context)

            # Second ResNet in this up block
            skip = skips.pop()
            x = torch.cat([x, skip], dim=1)
            x = block2(x, context=context)
        ##################################################################
