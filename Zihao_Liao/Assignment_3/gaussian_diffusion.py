...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        x_start = (x_t - sqrt_one_minus * noise) / sqrt_alphas_cumprod
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        sqrt_alphas_cumprod = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrt_one_minus = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - sqrt_alphas_cumprod * x_start) / sqrt_one_minus
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        model_out = self.model(x=x_t, time=t_batch, model_kwargs=model_kwargs)

        if self.objective == "pred_noise":
            # 模型输出的是噪声，先恢复 x_start
        ...
        # 4. 根据 q(x_{t-1} | x_t, x_0) 的后验分布采样
        posterior_mean, posterior_std = self.q_posterior(x_start, x_t, t_batch)
    ...
    def q_sample(self, x_start, t, noise):
        ...
        x_t = sqrt_alphas_cumprod * x_start + sqrt_one_minus * noise
        ####################################################################
        return x_t
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        model_out = self.model(x=x_t, time=t, model_kwargs=model_kwargs)  # (b, *)

        mse = (model_out - target) ** 2  # (b, *)
        # 对每个样本做加权，再对 batch 取平均
        loss = (loss_weight * mse).reshape(b, -1).mean(dim=-1).mean()
        ####################################################################
