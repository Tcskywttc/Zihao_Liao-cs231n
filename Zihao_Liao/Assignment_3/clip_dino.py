...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    # Normalize text and image features along the feature dimension
    text_norm = text_features / text_features.norm(dim=1, keepdim=True).clamp(min=1e-8)
    image_norm = image_features / image_features.norm(dim=1, keepdim=True).clamp(min=1e-8)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    # 1. Encode class texts
    text_tokens = clip.tokenize(class_texts).to(device)           # (C, L)
    text_features = clip_model.encode_text(text_tokens)           # (C, D)
    text_features = text_features / text_features.norm(dim=1, keepdim=True)

    # 2. Preprocess and encode images
    img_tensors = [clip_preprocess(Image.fromarray(im)) for im in images]
    img_batch = torch.stack(img_tensors, dim=0).to(device)        # (B, 3, H, W)
    image_features = clip_model.encode_image(img_batch)           # (B, D)
    image_features = image_features / image_features.norm(dim=1, keepdim=True)

    # 3. Compute similarity: text(C,D) vs image(B,D) -> (C, B)
    similarity = text_features @ image_features.t()               # (C, B)

    # 4. For each image, pick the best class
    best_class_indices = similarity.argmax(dim=0).tolist()        # length B
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # Precompute image features once
        self.clip_model.eval()
        img_tensors = [self.clip_preprocess(Image.fromarray(im)) for im in images]
        img_batch = torch.stack(img_tensors, dim=0).to(device)    # (N, 3, H, W)
        with torch.no_grad():
            image_features = self.clip_model.encode_image(img_batch)  # (N, D)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        # 2. Cosine similarity with precomputed image features
        # image_features: (N, D), text_features: (1, D)
        similarity = text_features @ self.image_features.t()          # (1, N)

        # 3. Top-k indices
        values, indices = similarity[0].topk(k)                       # (k,)
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        # Very small MLP: D -> 256 -> num_classes
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for _ in range(num_iters):
            ...
            loss = self.criterion(logits, Y_train)   # CE loss
            loss.backward()
            self.optimizer.step()
        ############################################################################
        #                             END OF YOUR CODE                             #
