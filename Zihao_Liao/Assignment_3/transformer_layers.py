...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Split into multiple heads: (N, S/T, E) -> (N, S/T, H, E/H) -> (N, H, S/T, E/H)
        Q = Q.view(N, S, H, head_dim).transpose(1, 2)  # (N, H, S, E/H)
        K = K.view(N, T, H, head_dim).transpose(1, 2)  # (N, H, T, E/H)
        V = V.view(N, T, H, head_dim).transpose(1, 2)  # (N, H, T, E/H)
        
        # Calculate attention scores
        scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(head_dim)  # (N, H, S, T)
        
        # Apply attention mask if provided
        # attn_mask[i,j] == 0 means token i should NOT attend to token j
        if attn_mask is not None:
            # Expand mask to match scores dimensions: (S, T) -> (1, 1, S, T)
            mask_expanded = attn_mask.unsqueeze(0).unsqueeze(0)
            # Where mask is 0 (False), set scores to -inf
        ...
        # Concatenate heads: (N, H, S, E/H) -> (N, S, H, E/H) -> (N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        P = self.patch_size
        num_patches_per_dim = H // P
        
        # Reshape to (N, C, num_patches_h, P, num_patches_w, P)
        x = x.reshape(N, C, num_patches_per_dim, P, num_patches_per_dim, P)
        
        # Permute to (N, num_patches_h, num_patches_w, C, P, P)
